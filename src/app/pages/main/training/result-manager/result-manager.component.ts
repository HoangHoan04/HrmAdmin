...
import { PagedResult, TrainingResult } from '@/app/core/models';
...
export class ResultManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'training.result.entityName';
  data: TrainingResult[] = [];
  loading = false;
  ...
  filterFields: FilterField[] = [
    {
      key: 'searchText',
      label: 'training.result.search',
      type: 'input',
      placeholder: 'training.result.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
  ];
  ...
  columns: TableColumn[] = [
    ...
    { field: 'score', header: 'training.result.score', type: 'number' },
    { field: 'grade', header: 'training.result.grade', type: 'text' },
    { field: 'completedAt', header: 'training.result.completedAt', type: 'date' },
  ];
    ...
    this.toolbarActions = [
      {
        ...
        visible: () => this.permissionSvc.has(PERMISSION_CODES.TRAINING_RESULT_MANAGE),
      },
    ];
    this.rowActions = [
      {
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<TrainingResult>>(this.apiService.TRAINING_RESULT.PAGINATION, payload)
      .subscribe({
        next: (res) => {
          this.data = res.items;
  ...
  openEdit(item: TrainingResult): void {
    this.router.navigate([
      ROUTES_CONFIG.TALENT.children.TRAINING.children.RESULT.children.EDIT_RESULT.path,
      item.id,
    ]);
  }
  delete(item: TrainingResult): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('training.result.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService
            .post<boolean>(this.apiService.TRAINING_RESULT.DELETE, { id: item.id })
            .subscribe({
              next: (ok) => {
                if (ok) {
