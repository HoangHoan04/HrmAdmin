...
export type AttendanceStatusCode = 'ON_TIME' | 'LATE' | 'EARLY' | 'LEAVE' | 'ABSENT' | 'INCOMPLETE';
...
export function resolveAttendanceStatus(status?: string | null): AttendanceStatusItem | null {
  if (!isAttendanceStatusCode(status)) return null;
  return BY_CODE[status];
}

export function getAttendanceStatusLabelKey(status?: string | null): string | null {
  return resolveAttendanceStatus(status)?.labelKey ?? null;
}

export const ATTENDANCE_STATUS_OPTIONS = Object.values(enumData.ATTENDANCE_STATUS);
