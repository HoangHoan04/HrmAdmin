...
export class LeaveManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'timeAttendance.leave.entityName';
  ...
  filterFields: FilterField[] = [
    {
      key: 'employeeId',
      label: 'timeAttendance.leave.employee',
      type: 'select',
      placeholder: 'timeAttendance.leave.filterEmployee',
      col: 6,
      allowClear: true,
      options: [],
    },
    {
      key: 'branchId',
      label: 'timeAttendance.leave.branch',
      type: 'select',
      placeholder: 'timeAttendance.leave.filterBranch',
      col: 6,
      allowClear: true,
      options: [],
    },
    {
      key: 'status',
      label: 'timeAttendance.leave.status',
      type: 'select',
      placeholder: 'timeAttendance.leave.filterStatus',
      col: 6,
      allowClear: true,
      options: [
        { label: 'timeAttendance.leave.statusPending', value: 'PENDING' },
        { label: 'timeAttendance.leave.statusApproved', value: 'APPROVED' },
        { label: 'timeAttendance.leave.statusRejected', value: 'REJECTED' },
        { label: 'timeAttendance.leave.statusCancelled', value: 'CANCELLED' },
      ],
    },
    {
      key: 'dateRange',
      label: 'timeAttendance.leave.filterDateRange',
      type: 'dateRange',
      col: 6,
      allowClear: true,
    },
  ...
  columns: TableColumn[] = [
    { field: 'employeeCode', header: 'timeAttendance.leave.employeeCode', type: 'text' },
    { field: 'employeeName', header: 'timeAttendance.leave.employee', type: 'text' },
    { field: 'dayOffType', header: 'timeAttendance.leave.dayOffType', type: 'text' },
    { field: 'dayOffConfigName', header: 'timeAttendance.leave.dayOffConfig', type: 'text' },
    { field: 'fromDate', header: 'timeAttendance.leave.fromDate', type: 'date' },
    { field: 'toDate', header: 'timeAttendance.leave.toDate', type: 'date' },
    { field: 'totalDays', header: 'timeAttendance.leave.totalDays', type: 'text' },
    { field: 'status', header: 'timeAttendance.leave.status', type: 'text' },
    { field: 'branchName', header: 'timeAttendance.leave.branch', type: 'text' },
    { field: 'reason', header: 'timeAttendance.leave.reason', type: 'text' },
  ];
  ...
  rowActions: RowAction[] = [
    {
      key: 'approve',
      icon: 'check-circle',
      tooltip: 'timeAttendance.leave.approve',
      severity: 'success',
    ...
    },
    {
      key: 'reject',
      icon: 'close-circle',
      tooltip: 'timeAttendance.leave.reject',
      severity: 'danger',
      visible: (record) => record.status === 'PENDING',
  ...
  approve(item: RegisterDayOff): void {
    this.promptNote('timeAttendance.leave.approve').then((note) => {
  ...
  reject(item: RegisterDayOff): void {
    this.promptNote('timeAttendance.leave.reject').then((note) => {
      if (note === undefined) return;
      this.apiService
    ...
    return new Promise((resolve) => {
      ...
      this.modal.confirm({
        ...
        nzContent: `<textarea id="leave-approver-note" class="ant-input" rows="3" placeholder="${this.i18n.instant(
          'timeAttendance.leave.approverNotePlaceholder',
        )}"></textarea>`,
        nzOnOk: () => {
