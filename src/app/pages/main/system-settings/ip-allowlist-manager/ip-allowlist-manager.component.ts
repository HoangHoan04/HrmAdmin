...
@Component({
  standalone: false,
  selector: 'app-ip-allowlist-manager',
  templateUrl: './ip-allowlist-manager.component.html',
  styleUrls: [],
})
export class IpAllowlistManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'system.ipAllowlist.entityName';
  data: IpAllowlistEntry[] = [];
  loading = false;
  ...
  filterFields: FilterField[] = [
    {
      ...
      type: 'input',
      placeholder: 'system.ipAllowlist.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
    {
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<IpAllowlistEntry>>(this.apiService.IP_ALLOWLIST.PAGINATION, payload)
      .subscribe({
        next: (res) => {
          this.data = res.items;
  ...
  openEdit(item: IpAllowlistEntry): void {
    this.router.navigate([
      ROUTES_CONFIG.SETTING_SYSTEM.children.IP_ALLOWLIST.children.EDIT_IP_ALLOWLIST.path,
      item.id,
    ]);
  }
  delete(item: IpAllowlistEntry): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('system.ipAllowlist.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService
            .post<boolean>(this.apiService.IP_ALLOWLIST.DELETE, { id: item.id })
            .subscribe({
              next: (ok) => {
                if (ok) {
