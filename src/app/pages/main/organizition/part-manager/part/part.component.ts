...
export class PartComponent implements OnInit {
  ...
  toolbarActions: TableAction[] = [
    ...
    },
    {
      ...CommonActions.uploadExcel({
        templateUrl: () => this.apiService.PART.EXCEL_TEMPLATE,
        importUrl: () => this.apiService.PART.EXCEL_IMPORT,
  ...
  exportExcel() {
    ...
    return this.apiService.postBlob(this.apiService.PART.EXCEL_EXPORT, payload).pipe(
      tap({
        next: (response) => {
          this.excelLoading = false;
          const blob = response.body;
