...
import { PagedResult, TrainingEnrollment } from '@/app/core/models';
...
export class EnrollmentManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'training.enrollment.entityName';
  ...
  filterFields: FilterField[] = [
    {
      key: 'searchText',
      label: 'training.enrollment.search',
      type: 'input',
      placeholder: 'training.enrollment.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
    {
      key: 'status',
      label: 'training.enrollment.status',
      type: 'select',
      placeholder: 'training.common.filterStatus',
      col: 8,
      allowClear: true,
      options: Object.values(enumData.TRAINING_ENROLLMENT_STATUS).map((x) => ({
        label: x.labelKey,
        value: x.value,
      })),
    },
  ...
  columns: TableColumn[] = [
    { field: 'courseName', header: 'training.enrollment.course', type: 'text' },
    { field: 'employeeName', header: 'training.enrollment.employee', type: 'text' },
    { field: 'enrolledAt', header: 'training.enrollment.enrolledAt', type: 'date' },
    { field: 'status', header: 'training.enrollment.status', type: 'text' },
  ];
    ...
    this.toolbarActions = [
      {
        ...
        visible: () => this.permissionSvc.has(PERMISSION_CODES.TRAINING_ENROLLMENT_MANAGE),
      },
    ];
    this.rowActions = [
      {
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<TrainingEnrollment>>(
        this.apiService.TRAINING_ENROLLMENT.PAGINATION,
        payload,
      )
      .subscribe({
        next: (res) => {
  ...
  openCreate(): void {
    this.router.navigate([
      ROUTES_CONFIG.TALENT.children.TRAINING.children.ENROLLMENT.children.ADD_ENROLLMENT.path,
    ]);
  }
  openEdit(item: TrainingEnrollment): void {
    this.router.navigate([
      ROUTES_CONFIG.TALENT.children.TRAINING.children.ENROLLMENT.children.EDIT_ENROLLMENT.path,
      item.id,
    ]);
  }
  delete(item: TrainingEnrollment): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('training.enrollment.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService
            .post<boolean>(this.apiService.TRAINING_ENROLLMENT.DELETE, { id: item.id })
            .subscribe({
              next: (ok) => {
                if (ok) {
