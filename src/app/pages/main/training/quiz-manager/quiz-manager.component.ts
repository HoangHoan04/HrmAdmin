...
import { PagedResult, TrainingQuiz } from '@/app/core/models';
...
export class QuizManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'training.quiz.entityName';
  data: TrainingQuiz[] = [];
  loading = false;
  ...
  filterFields: FilterField[] = [
    {
      key: 'searchText',
      label: 'training.quiz.search',
      type: 'input',
      placeholder: 'training.quiz.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
  ];
  ...
  columns: TableColumn[] = [
    { field: 'courseName', header: 'training.quiz.course', type: 'text' },
    { field: 'question', header: 'training.quiz.question', type: 'text' },
    { field: 'correctOption', header: 'training.quiz.correctOption', type: 'text' },
  ];
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<TrainingQuiz>>(this.apiService.TRAINING_QUIZ.PAGINATION, payload)
      .subscribe({
        next: (res) => {
          this.data = res.items;
  ...
  delete(item: TrainingQuiz): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('training.quiz.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService
            .post<boolean>(this.apiService.TRAINING_QUIZ.DELETE, { id: item.id })
            .subscribe({
              next: (ok) => {
                if (ok) {
