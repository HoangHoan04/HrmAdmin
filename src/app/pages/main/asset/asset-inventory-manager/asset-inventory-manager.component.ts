...
@Component({
  standalone: false,
  selector: 'app-asset-inventory-manager',
  templateUrl: './asset-inventory-manager.component.html',
  styleUrls: [],
})
export class AssetInventoryManagerComponent implements OnInit {
  ...
  filterFields: FilterField[] = [
    {
      key: 'searchText',
      label: 'asset.inventory.search',
      type: 'input',
      placeholder: 'asset.inventory.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
    {
      ...
      col: 8,
      allowClear: true,
      options: Object.values(enumData.ASSET_STATUS).map((x) => ({
        label: x.labelKey,
        value: x.value,
      })),
    },
  ...
  openEdit(item: Asset): void {
    this.router.navigate([
      ROUTES_CONFIG.ASSET.children.ASSET_MANAGER.children.EDIT_ASSET.path,
      item.id,
    ]);
  }
  openDetail(item: Asset): void {
    this.router.navigate([
      ROUTES_CONFIG.ASSET.children.ASSET_MANAGER.children.DETAIL_ASSET.path,
      item.id,
    ]);
  }
  delete(item: Asset): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('asset.inventory.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService.post<boolean>(this.apiService.ASSET.DELETE, { id: item.id }).subscribe({
            next: (ok) => {
              if (ok) {
