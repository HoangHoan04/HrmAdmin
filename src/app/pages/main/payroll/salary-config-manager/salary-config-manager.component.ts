...
@Component({
  standalone: false,
  selector: 'app-salary-config-manager',
  templateUrl: './salary-config-manager.component.html',
  styleUrls: [],
})
export class SalaryConfigManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'salaryConfig.entityName';
  ...
  toolbarActions: TableAction[] = [
    {
      ...
      visible: () => this.permissionSvc.has(PERMISSION_CODES.PAYROLL_CONFIG_CREATE),
    },
  ];
  ...
  filterFields: FilterField[] = [
    {
      key: 'code',
      label: 'salaryConfig.code',
      type: 'input',
      placeholder: 'salaryConfig.searchCode',
      col: 6,
      allowClear: true,
    },
    {
      key: 'name',
      label: 'salaryConfig.name',
      type: 'input',
      placeholder: 'salaryConfig.searchName',
      col: 6,
      allowClear: true,
    },
    {
      key: 'companyId',
      label: 'salaryConfig.companyName',
      type: 'select',
      placeholder: 'salaryConfig.filterCompany',
      col: 6,
      allowClear: true,
      options: [],
    },
  ...
  columns: TableColumn[] = [
    { field: 'code', header: 'salaryConfig.code', type: 'text', sortable: true },
    { field: 'name', header: 'salaryConfig.name', type: 'text', sortable: true },
    { field: 'companyName', header: 'salaryConfig.companyName', type: 'text' },
    {
      field: 'standardWorkingDays',
      header: 'salaryConfig.standardWorkingDays',
      type: 'number',
    },
  ...
  rowActions: RowAction[] = [
    {
      ...
      severity: 'info',
      visible: () => this.permissionSvc.has(PERMISSION_CODES.PAYROLL_CONFIG_UPDATE),
    ...
    },
    {
      ...
      visible: (record) =>
        ...
        this.permissionSvc.has(PERMISSION_CODES.PAYROLL_CONFIG_ACTIVATE),
    ...
    },
    {
      ...
      visible: (record) =>
        ...
        this.permissionSvc.has(PERMISSION_CODES.PAYROLL_CONFIG_DEACTIVATE),
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<SalaryConfig>>(this.apiService.SALARY_CONFIG.PAGINATION, payload)
      .subscribe({
        next: (res) => {
          this.data = res.items;
  ...
  openEdit(item: SalaryConfig): void {
    this.router.navigate([
  ...
  async activate(item: SalaryConfig): Promise<void> {
    if (!item.id) return;
    const confirmed = await this.actionConfirm.confirmActivate(this.ENTITY_KEY, item.name);
    if (!confirmed) return;
    this.apiService
      .post<boolean>(this.apiService.SALARY_CONFIG.ACTIVATE, { id: item.id })
      .subscribe({
        next: (success) => {
          if (success) {
  ...
  async deactivate(item: SalaryConfig): Promise<void> {
    if (!item.id) return;
    const confirmed = await this.actionConfirm.confirmDeactivate(this.ENTITY_KEY, item.name);
    if (!confirmed) return;
    this.apiService
      .post<boolean>(this.apiService.SALARY_CONFIG.DEACTIVATE, { id: item.id })
      .subscribe({
        next: (success) => {
          if (success) {
