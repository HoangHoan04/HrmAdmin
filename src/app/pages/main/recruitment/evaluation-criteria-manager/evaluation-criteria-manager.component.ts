...
@Component({
  standalone: false,
  selector: 'app-evaluation-criteria-manager',
  templateUrl: './evaluation-criteria-manager.component.html',
  styleUrls: [],
})
export class EvaluationCriteriaManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'recruitment.criteria.entityName';
  ...
  filterFields: FilterField[] = [
    {
      key: 'searchText',
      label: 'recruitment.criteria.search',
      type: 'input',
      placeholder: 'recruitment.criteria.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
    {
      key: 'category',
      label: 'recruitment.criteria.category',
      type: 'input',
      placeholder: 'recruitment.criteria.category',
      col: 8,
      allowClear: true,
    },
    {
  ...
  columns: TableColumn[] = [
    { field: 'code', header: 'recruitment.criteria.code', type: 'text', sortable: true },
    { field: 'name', header: 'recruitment.criteria.name', type: 'text', sortable: true },
    ...
    this.toolbarActions = [
      {
        ...
        visible: () => this.permissionSvc.has(PERMISSION_CODES.RECRUITMENT_CRITERIA_MANAGE),
      },
    ];
    this.rowActions = [
      {
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<EvaluationCriteria>>(
        this.apiService.EVALUATION_CRITERIA.PAGINATION,
        payload,
      )
      .subscribe({
        next: (res) => {
  ...
  onFilterClear(): void {
    this.filters = { searchText: '', category: '', isActive: null };
    this.pagination.current = 1;
    this.loadData();
  }
  ...
  openEdit(item: EvaluationCriteria): void {
    this.router.navigate([
      ROUTES_CONFIG.RECRUITMENT.children.SETUP.children.EVALUATION_CRITERIA.children
  ...
  delete(item: EvaluationCriteria): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('recruitment.criteria.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService
            .post<boolean>(this.apiService.EVALUATION_CRITERIA.DELETE, { id: item.id })
            .subscribe({
              next: (ok) => {
                if (ok) {
