...
import { AssetType, CompanySelectBoxDto } from '@/app/core/models';
...
@Component({
  standalone: false,
  selector: 'app-add-or-update-asset-type',
  templateUrl: './add-or-update-asset-type.component.html',
  styleUrls: [],
})
export class AddOrUpdateAssetTypeComponent implements OnInit {
  id: string | null = null;
  isEdit = false;
  loading = false;
  submitting = false;
  ...
  loadDetail(id: string): void {
    this.loading = true;
    this.apiService.post<AssetType>(this.apiService.ASSET_TYPE.DETAIL, { id }).subscribe({
      next: (item) => {
  ...
  goBack(): void {
    this.router.navigate([ROUTES_CONFIG.ASSET.children.ASSET_TYPE.path]);
  }
  ...
  submitForm(): void {
    ...
    const endpoint = this.isEdit
      ? this.apiService.ASSET_TYPE.UPDATE
      : this.apiService.ASSET_TYPE.CREATE;
