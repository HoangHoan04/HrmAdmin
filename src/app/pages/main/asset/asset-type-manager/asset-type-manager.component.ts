...
@Component({
  standalone: false,
  selector: 'app-asset-type-manager',
  templateUrl: './asset-type-manager.component.html',
  styleUrls: [],
})
export class AssetTypeManagerComponent implements OnInit {
  ...
  filterFields: FilterField[] = [
    {
      key: 'searchText',
      label: 'asset.type.search',
      type: 'input',
      placeholder: 'asset.type.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
    {
      key: 'isActive',
      label: 'asset.common.status',
      type: 'select',
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<AssetType>>(this.apiService.ASSET_TYPE.PAGINATION, payload)
      .subscribe({
        next: (res) => {
          this.data = res.items;
  ...
  openEdit(item: AssetType): void {
    this.router.navigate([
      ROUTES_CONFIG.ASSET.children.ASSET_TYPE.children.EDIT_ASSET_TYPE.path,
      item.id,
    ]);
  }
  delete(item: AssetType): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('asset.type.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService
            .post<boolean>(this.apiService.ASSET_TYPE.DELETE, { id: item.id })
            .subscribe({
              next: (ok) => {
                if (ok) {
