...
import { PagedResult, WebhookSubscription } from '@/app/core/models';
...
export class WebhookManagerComponent implements OnInit {
  private readonly ENTITY_KEY = 'system.webhook.entityName';
  ...
  filterFields: FilterField[] = [
    {
      ...
      type: 'input',
      placeholder: 'system.webhook.searchPlaceholder',
      col: 8,
      allowClear: true,
    },
    {
  ...
  columns: TableColumn[] = [
    { field: 'name', header: 'system.webhook.name', type: 'text', sortable: true },
    { field: 'url', header: 'system.webhook.url', type: 'text' },
    { field: 'eventTypes', header: 'system.webhook.eventTypes', type: 'text' },
  ...
  loadData(): void {
    ...
    this.apiService
      .post<PagedResult<WebhookSubscription>>(
        this.apiService.WEBHOOK_SUBSCRIPTION.PAGINATION,
        payload,
      )
      .subscribe({
        next: (res) => {
  ...
  openEdit(item: WebhookSubscription): void {
    this.router.navigate([
      ROUTES_CONFIG.SETTING_SYSTEM.children.WEBHOOKS.children.EDIT_WEBHOOK.path,
      item.id,
    ]);
  }
  delete(item: WebhookSubscription): void {
    this.modal.confirm({
      nzTitle: this.i18n.instant('system.webhook.deleteConfirm'),
      nzOnOk: () =>
        new Promise<void>((resolve, reject) => {
          this.apiService
            .post<boolean>(this.apiService.WEBHOOK_SUBSCRIPTION.DELETE, { id: item.id })
            .subscribe({
              next: (ok) => {
                if (ok) {
